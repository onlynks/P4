...
public class ComptabiliteManagerImpl extends AbstractBusinessManager implements ComptabiliteManager {
	...
	public synchronized void addReference(EcritureComptable pEcritureComptable) {
		...
		SequenceEcritureComptable sequenceEcriture;

		try {
			sequenceEcriture = getDaoProxy().getComptabiliteDao().getSequenceEcritureComptable(code, annee);

			derniereValeur = sequenceEcriture.getDerniereValeur() + 1;
			sequenceEcriture.setDerniereValeur(derniereValeur);

		} catch (NotFoundException e) {

			sequenceEcriture = new SequenceEcritureComptable();
			derniereValeur = 1;

			sequenceEcriture.setCodeJournal(code);
			sequenceEcriture.setAnnee(annee);
			sequenceEcriture.setDerniereValeur(derniereValeur);
		}
		try {
			if (sequenceEcriture.getDerniereValeur() == 1) {
				getDaoProxy().getComptabiliteDao().createSequenceEcritureComptable(sequenceEcriture);
			} else {
	...
	public void checkEcritureComptableUnit(EcritureComptable pEcritureComptable) throws FunctionalException {
		...
		if (!annee.equals(anneeRef)) {
			throw new FunctionalException("La référence doit contenir l'année correspondant à la date de l'écriture");
		}
	}
